...
in eval/qa_pairs.json. If GEMINI_API_KEY is available, can also evaluate
...
QA_PAIRS_PATH = os.path.join(os.path.dirname(__file__), "qa_pairs.json")
...
def evaluate_retrieval(top_k: int = 5):
    ...
    for item in qa_pairs:
        ...
        retrieved_articles = [r["article_number"] for r in retrieved]

        is_top1 = bool(retrieved_articles and retrieved_articles[0] in expected)
        is_topk = any(art in expected for art in retrieved_articles)
        ...
        status = "[PASS]" if is_topk else "[FAIL]"
        rank = (
    ...
    total = len(qa_pairs)
    top1_pct = (hits_top1 / total) * 100
    topk_pct = (hits_topk / total) * 100
